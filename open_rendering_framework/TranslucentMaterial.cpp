...
TranslucentMaterial::TranslucentMaterial(optix::Context c)
{
	context = c;
	type = TRANSLUCENT_SHADER;
	shader_name = QString::fromStdString("subsurface_scattering_shader.cu");
	//concentration = 1.0f;
	scale = 100.0f;
...
TranslucentMaterial::TranslucentMaterial(optix::Context c, const QJsonObject &json)
{
	context = c;
	type = TRANSLUCENT_SHADER;
	dipole_model = STANDARD_DIPOLE;
...
void TranslucentMaterial::initTable()
{
	...
	QObject::connect(table, &QTableWidget::cellChanged, this, &TranslucentMaterial::tableUpdate);
};

void TranslucentMaterial::initPrograms()
{
...
void TranslucentMaterial::tableUpdate(int row, int column)
{
	if (row == 1)
...
void TranslucentMaterial::loadFromJSON(const QJsonObject &json)
{
...
void TranslucentMaterial::writeJSON(QJsonObject &json) const
{
...
void TranslucentMaterial::getDefaultMaterial(DefaultScatteringMaterial material)
{
	current = material;
...
void TranslucentMaterial::loadParameters(const char* name)
{
	mtl[name]->setUserData(sizeof(ScatteringMaterialProperties), &properties);
	mtl["dipole_model"]->setUint(dipole_model);
}

void TranslucentMaterial::computeCoefficients(optix::float3 ior_outside)
{
...
void TranslucentMaterial::updateIORFromTable(int row)
{
...
void TranslucentMaterial::updateScaleFromTable(int row)
{
	float s = table->item(row, 1)->text().toFloat();
	setScale(s);
}

void TranslucentMaterial::updateAbsorptionFromTable(int row)
{
...
void TranslucentMaterial::updateScatteringFromTable(int row)
{
	float x = table->item(row, 1)->text().toFloat();
	float y = table->item(row, 2)->text().toFloat();
	float z = table->item(row, 3)->text().toFloat();
...
void TranslucentMaterial::updateMeanCosineFromTable(int row)
{
	float x = table->item(row, 1)->text().toFloat();
	float y = table->item(row, 2)->text().toFloat();
	float z = table->item(row, 3)->text().toFloat();
...
void TranslucentMaterial::setIor(float i)
{
	ior = i;
	computeCoefficients();
	loadParameters("scattering_properties");
...
void TranslucentMaterial::setScale(float s)
{
	scale = s;
	computeCoefficients();
	loadParameters("scattering_properties");
...
void TranslucentMaterial::setAbsorption(optix::float3 a)
{
	absorption = a;
	computeCoefficients();
	loadParameters("scattering_properties");
...
void TranslucentMaterial::setScattering(optix::float3 s)
{
	scattering = s;
	computeCoefficients();
	loadParameters("scattering_properties");
...
void TranslucentMaterial::setMeanCosine(optix::float3 m)
{
	meancosine = m;
	computeCoefficients();
	loadParameters("scattering_properties");
...
void TranslucentMaterial::updateDefaultMaterial(int material)
{
...
void TranslucentMaterial::updateDipoleModel(int model)
{
