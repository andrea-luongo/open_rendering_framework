...
_glmReadMTL(GLMmodel* model, char* name)
{
  ...
  for (i = 0; i < nummaterials; i++) {
    ...
    model->materials[i].ior       = 1.0f;
    model->materials[i].refraction = 1.0f;
    model->materials[i].alpha      = 1.0f;
    model->materials[i].shader     = GLM_FLAT_SHADE;
    model->materials[i].reflectivity = 0.0f;
...
/* glmReadMTL: Reads materials from a Wavefront .MTL file.
 * Returns a pointer to the created object which should be free'd with
 * glmDelete().
 *
 * filename - name of the file containing the Wavefront .MTL format data.
 *
 * returns 0 if there was a problem reading the file.
 */
  GLMmodel* 
glmReadMTL(const char* filename)
{
  GLMmodel* model;
  FILE*     file;
  ...
  file = fopen(filename, "r");
  if (!file) {
    perror("glmReadMTL() failed: can't open data file");
    return 0;
  }
  ...
  model = (GLMmodel*)malloc(sizeof(GLMmodel));
  model->pathname      = NULL;
  model->mtllibname    = strdup(filename);
  model->numvertices   = 0;
  model->vertices      = NULL;
  model->numnormals    = 0;
  model->normals       = NULL;
  ...
  if (_glmReadMTL(model, model->mtllibname)) {
    /* There was a problem here, so cleanup and exit. */
    glmDelete(model);
    fclose(file);
    return 0;
