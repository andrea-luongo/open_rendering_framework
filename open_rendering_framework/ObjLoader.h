...
public:
   ...
   ~ObjLoader() {} // makes sure CRT objects are destroyed on the correct heap
	...
	void loadAreaLight(const optix::float3 radiance);
	void loadAreaLight(const optix::Matrix4x4& transform, const optix::float3 radiance);
...
private:
	...
	void createLightBuffer(GLMmodel* model, const optix::float3 radiance, const optix::Matrix4x4 transform);
	optix::TextureSampler createConstantTexture(optix::float3 default_color);
